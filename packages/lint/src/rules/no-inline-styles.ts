...
// Unwraps `as CSSProperties`, `satisfies`, and `!`.
...
// The declarations a static style attribute carries: `color: red;
// padding: 4px`. Semicolons inside quotes or parentheses do not split.
function staticStylePairs(text: string) {
  ...
  const patterns = (entries ?? []).map((entry) => {
    ...
    if (!custom && !/^[a-zA-Z*][a-zA-Z*-]*$/.test(entry)) {
      throw new ContractConfigError(
        `shadcn-vue/no-inline-styles: entry "${entry}" is not a CSS property name (backgroundColor, background-color, border-*, --chart-1), so it would match nothing.`
      )
    }
...
export const noInlineStyles = {
  ...
  create(context: any) {
    ...
    const emit = reporter(context, MESSAGES, {
      rule: "shadcn-vue/no-inline-styles",
    ...
    // The tag as written (Text, MyButton). A lowercase or kebab-case
    // tag is intrinsic and takes the top-level policy.
    const componentOf = (attribute: any): string => {
      const element = attribute?.parent?.parent
      if (element?.type !== "VElement") return ""
      const rawName = typeof element.rawName === "string" ? element.rawName : ""
      return /^[A-Z]/.test(rawName) ? rawName : ""
    }
    // Template expressions read the script-setup scope through the same
    // shim the class sites use.
    const reader = templateContextFor(context)
    ...
    const check = (
      ...
      expr = unwrap(expr)
      if (!expr) return
      const forwarded = forwardedValuesOf(expr, reader, "style", seen)
      ...
      if (expr.type === "Identifier") {
        const init = resolveIdentifier(expr, reader, seen)?.init
      ...
      if (expr.type === "MemberExpression") {
        const found = resolveMemberValue(expr, reader, seen)
      ...
      for (const entry of objectEntries(expr, reader, seen)) {
        if ("unknown" in entry) {
          const prop = entry.unknown
          if (
            prop.type === "SpreadElement" &&
            isForwardedProp(unwrap(prop.argument), reader, "style")
          ) {
      ...
      for (const [key, value] of properties) {
        ...
        } else if (carriesRawColor(value, reader)) {
          emit(
            {
              node: value,
    ...
    const templateVisitor = {
      VAttribute(node: any) {
        const key = node.key
        if (!node.directive) {
          if (key?.name !== "style") return
          const literal = node.value
          if (literal?.type !== "VLiteral" || typeof literal.value !== "string")
            return
          // CSS as written: the declarations a static attribute carries,
          // each judged like a style property.
          for (const { key, value } of staticStylePairs(literal.value)) {
            const verdict = policy.decide(componentOf(node), key)
            if (verdict.exempt) continue
            if (!key.startsWith("--")) {
              emit(
                {
                  node,
                  messageId: "inlineStyle",
                  data: { property: key, component: componentOf(node) },
                },
                verdict.message
              )
            } else if (hasRawColor(value)) {
              emit(
                {
                  node,
                  messageId: "customPropColor",
        ...
        const argument = key.argument?.name
        if (argument === "style") {
          const expression = node.value?.expression
          if (!expression) return
          check(expression, expression, new Set(), componentOf(node))
          return
        }
        if (argument) return
