...
function noteFor(context: any) {
  const note = context.settings?.["shadcn-vue"]?.note
  if (note == null) return ""
  ...
  for (const [, key] of message.matchAll(PLACEHOLDER)) {
    ...
    if (nearest) {
      warnOnce(
        ...
        `Unknown message placeholder "{{${key}}}" in ${label.startsWith("shadcn-vue/") ? label : `contract "${label}"`}. Did you mean "{{${nearest}}}"? It will remain literal.`
      )
    }
  }
}
