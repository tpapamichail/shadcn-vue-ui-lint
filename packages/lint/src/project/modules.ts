...
import { sfcComponentName } from "./sfc"
...
function parseModule(file: string): ParsedModule {
  ...
  const defaultIdentifier = source.match(DEFAULT_IDENTIFIER_RE)
  if (defaultDeclaration) module.defaultName = defaultDeclaration[1]
    ...
    // A component file has no `export default` in its text: the
    // component is the file. Not added to local — the name is the
    // file's, not a binding in it.
    module.defaultName = sfcComponentName(file)
  }
